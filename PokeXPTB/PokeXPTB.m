...
for currentTrial = 1:MaxTrials
    ...
    imageTextureX = Screen('MakeTexture', window1, imX);

    %Image textures global so SoftCodeHandler function can access
    global imageTextureLeft %#ok<TLEV>
    global imageTextureRight %#ok<TLEV>
    if TrialTypes(currentTrial) == 1 % X to be displayed over port 1
        StateOnLeftPoke = 'LeftReward'; StateOnRightPoke = 'Timeout';
        imageTextureLeft = imageTextureX;
        imageTextureRight = imageTextureGray;
    else % X to be displayed over port 2
        StateOnLeftPoke = 'Timeout'; StateOnRightPoke = 'RightReward';
        imageTextureLeft = imageTextureGray;
        imageTextureRight = imageTextureX;
    end
    ...
    sma = AddState(sma, 'Name', 'WaitForInit', ...
        'Timer', 0,...
        'StateChangeConditions', {InitPort, 'WaitForPoke'},...
